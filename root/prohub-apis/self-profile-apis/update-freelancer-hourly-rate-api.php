...
if (!Utilities::checkJsonKeysMatch($requestBody, ['freelancer_id', 'freelancer_hourly_rate'])) {
    HTTPHandler::sendResponse(
        400,
        false,
...
// Now just update the freelancer's hourly rate and return the success response.
try {
    $query = "UPDATE freelancers
              SET freelancer_hourly_rate = ?
              WHERE freelancer_id = ?";
    $queryResult = $databaseConnection->executeQuery(
        $query,
        [$freelancerHourlyRate, $freelancerId],
        'si'
    );
}
catch (Exception $exception) {
