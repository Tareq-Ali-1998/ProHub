...
/*
This query retrieves information about the user's posted jobs, returning various details related to each job.

To retrieve the required data, only the user's ID is needed.

Note that this query retrieves the uncompleted jobs as well as jobs that have already been completed by a freelancer.

The results of the query are ordered based on the job creation date, with the most recent jobs appearing first.

I am satisfied with the current time complexity, so to remind myself in the future, I can run the EXPLAIN
ANALYZE FORMAT = TREE query to check that this query is the best query that could be written in terms of
performance because it retrieves only the needed data without scanning any other data from any record that
is not involved in the final result set;).
*/
...
FROM 
  (
    ...
    FROM 
      ...
      LEFT JOIN `freelancing project`.`user_favorite_jobs` ON (`freelancing project`.`user_favorite_jobs`.user_id = 88 AND
                                                               `freelancing project`.`jobs`.job_id = `freelancing project`.`user_favorite_jobs`.job_id)
    WHERE 
      `freelancing project`.`jobs`.user_id = 88
    ORDER BY 
